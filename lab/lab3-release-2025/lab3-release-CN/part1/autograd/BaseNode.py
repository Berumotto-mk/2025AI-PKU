...
class Attention(Node):
    ...
    def __init__(self, dim):
        super().__init__("Attention")#调用父类构造函数
    ...
    def cal(self, X):
        ...
        V = X @ W_v # V(B, L ,d)
        #softmax_score = []
        E = Q @ K.transpose(0,2,1)/np.sqrt(self.dim)#(B,L,L)
        #E是一个(B,L,L)的矩阵
        exp_E = np.exp(E)
        softmax_score = exp_E / exp_E.sum(axis = -1,keepdims = True)#为(B,L,L)维度
        self.cache.append([X,softmax_score,Q,K,V])
        ret = softmax_score@V +X
        return ret
...
class Linear(Node):
    ...
    def backcal(self, grad):
        ...
        gradB = np.sum(grad,axis=0)#延竖直轴求和
...
class ResLinear(Node):
    ...
    def cal(self, X):
        ...
        # 即Linear(X)+X
        ret =np.dot(X,self.params[0])+self.params[1]
        output =ret + X #广播机制，输出维度为max(d1,d2)
        self.cache.append(X)
        return output
    ...
    def backcal(self, grad):
        # TODO: YOUR CODE HERE
        gradW = self.cache[-1].transpose(0,2,1)@grad
        ...
        return np.dot(grad,self.params[0].T)+grad
        raise NotImplementedError
