...
def generate_uniform_particles(walls, N):#初始化粒子
    """
    输入：
    walls: 维度为(xxx, 2)的np.array, 地图的墙壁信息，具体设定请看README关于地图的部分#walls中是否只有#，没有.
    N: int, 采样点数量
    输出：
...
def calculate_particle_weight(estimated, gt):#计算粒子权重
    """
    输入：
    estimated: np.array, 该采样点的距离传感器数据#距离传感器数据的内容
...
def apply_state_transition(p: Particle, traveled_distance, dtheta):
    ...
    p.theta=(p.theta+dtheta)%(2*np.pi) #将角度取余
    ### 你的代码 ###
    return p
...
def get_estimate_result(particles: List[Particle]):
    ...
    """
    final_result = Particle()#最有可能的粒子？
    ### 你的代码 ###
    final_result= max(particles, key=Particle.get_weight)#选取权重最大的那个
    ### 你的代码 ###
    return final_result