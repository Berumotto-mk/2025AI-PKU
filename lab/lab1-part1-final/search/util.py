...
class PriorityQueue:
    """
      ...
      # 实现优先级队列数据结构。每个插入的项目都有与之关联的优先级，
      #客户端通常对队列中优先级最低的项目的快速检索感兴趣。此数据结构允许对优先级最低的项目进行O(1)访问。
    """
    def  __init__(self):
        self.heap = []
        self.count = 0
    ...
    def push(self, item, priority):
        entry = (priority, self.count, item)#(优先级，计数，项目)元组
        heapq.heappush(self.heap, entry)#将entry插入到heap中
        self.count += 1

    def pop(self):
        (_, _, item) = heapq.heappop(self.heap)#弹出heap中的最小值
        return item
...
class PriorityQueueWithFunction(PriorityQueue):
    ...
    # 实现具有与Queue和Stack类相同的push/pop签名的优先级队列。这是为这两个类的替换而设计的。
...
def raiseNotDefined():#定义了一个函数，用于输出错误信息
    fileName = inspect.stack()[1][1]
    #获取调用该函数的文件名，inspect.stack()返回一个列表，
    #列表中的每个元素是一个元组，元组中包含了调用函数的信息
    line = inspect.stack()[1][2]
    #获取调用该函数的行号，inspect.stack()返回一个列表，
    #列表中的每个元素是一个元组，元组中包含了调用函数的信息
    method = inspect.stack()[1][3]
    #获取调用该函数的函数名，inspect.stack()返回一个列表，
    print("*** Method not implemented: %s at line %s of %s" % (method, line, fileName))
    #输出错误信息，提示用户该函数没有实现
    sys.exit(1)#sys.exit(1)是退出程序的意思
